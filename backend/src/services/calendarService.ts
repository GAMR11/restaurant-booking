import { getCalendarClient } from '../config/googleCalendar';
...
export class CalendarService {
   ...
   */
  async getEventsForDate(date: Date): Promise<calendar_v3.Schema$Event[]> {
    try {
   ...
   */
  private extractGuestsFromEvent(event: calendar_v3.Schema$Event): number {
   ...
   */
  private getSlotsBetween(startTime: Date, endTime: Date): string[] {
    const slots: string[] = [];
   ...
   */
  async getOccupiedSlots(date: Date): Promise<Map<string, number>> {
   ...
   */
  private createEcuadorDateTime(dateString: string, timeString: string): Date {
  ...
  async updateEvent(
    ...
    try {
      ...
      if (updates.numberOfGuests) {
        ...
        const updatedSummary = currentSummary.replace(
          /\d+\s*personas?/i,
          `${updates.numberOfGuests} personas`
        );
